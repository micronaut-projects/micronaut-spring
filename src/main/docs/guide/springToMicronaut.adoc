...
Unlikely traditional runtime reflection based frameworks Micronaut uses Ahead of Time (AOT) compilation, hence there is no runtime overhead to supporting an additional set of annotations (in this case Spring's annotation programming model).
...
* Existing tooling like IntelliJ IDE and STS 4.0 "Just Work" with Micronaut
