...
|link:{springapi}org/springframework/context/annotation/Fallback.html[@Fallback]
|link:{micronautapi}context/annotation/Secondary.html[@Secondary]
|Example `@Fallback`. Requires `micronaut-context` dependency.
